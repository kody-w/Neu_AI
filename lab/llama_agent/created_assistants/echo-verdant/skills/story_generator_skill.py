...
class StoryGeneratorSkill(BasicSkill):
    def __init__(self):
        ...
        self.metadata = {
            "name": self.name,
            "description": "Generates a complete story using Groq's API, following a multi-step process, and saves each stage to separate files. All aspects of the generation process are customizable through parameters.",
            "parameters": {
                "type": "object",
                "properties": {
                    "genre": {
                    ...
                    "temperature": {
                        "type": "number",
                        "description": "Controls randomness in generation. Higher values make output more random, lower values make it more focused",
                        "default": 0.7
                    },
                    "max_tokens": {
                        "type": "integer",
                        "description": "The maximum number of tokens to generate in each API call",
                        "default": 2000
                    }
                },
                "required": ["genre", "idea"]
    ...
    def perform(self, genre, idea, num_chapters=7, temperature=0.7, max_tokens=2000):
        try:
            ...
            story_idea = self.generate_story_idea(genre, idea, temperature, max_tokens)
            idea_file = self.save_to_file(story_dir, "1_story_idea.txt", story_idea)
            
            # Generate and save story outline
            story_outline = self.generate_story_outline(genre, idea_file, temperature, max_tokens)
            outline_file = self.save_to_file(story_dir, "2_story_outline.txt", story_outline)
            
            # Generate and save chapter summaries
            chapter_summaries = self.generate_chapter_summaries(genre, outline_file, temperature, max_tokens)
            summaries_file = self.save_to_file(story_dir, "3_chapter_summaries.txt", chapter_summaries)
            
            # Generate and save full story
            full_story = self.generate_full_story(genre, summaries_file, num_chapters, temperature, max_tokens)
            story_file = self.save_to_file(story_dir, "4_full_story.txt", full_story)
            
            return f"Story successfully generated and saved in {story_dir}\n\nHere's a summary of the generated story:\n\n{story_idea}"
        except Exception as e:
            return f"An error occurred while generating the story: {str(e)}"

    def groq_call(self, messages, temperature, max_tokens):
        response = self.chatbot.invoke(messages, temperature=temperature, max_tokens=max_tokens)
        return response.content

    def generate_story_idea(self, genre, idea, temperature, max_tokens):
        messages = [
            SystemMessage(content=f"You are a {genre} author. Your task is to write {genre} stories in a vivid and intriguing language."),
...
""")
        ...
        return self.groq_call(messages, temperature, max_tokens)

    def generate_story_outline(self, genre, idea_file, temperature, max_tokens):
        with open(idea_file, 'r', encoding='utf-8') as file:
            story_idea = file.read()

        messages = [
            SystemMessage(content=f"You are a {genre} author. Your task is to write {genre} stories in a vivid and intriguing language, always focus on a show dont tell writing style and include intriguing Dialog."),
            HumanMessage(content=f"""
...
""")
    ...
    def generate_chapter_summaries(self, genre, outline_file, temperature, max_tokens):
...
""")
    ...
    def generate_full_story(self, genre, summaries_file, num_chapters, temperature, max_tokens):
...
""")
            ...
            chapter_content = self.groq_call(messages, temperature, max_tokens)
